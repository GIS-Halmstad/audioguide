...
| `guideClickedInMap`       | `guideId: number, stopNumber?: number` | User clicked on guide in the map view. Also contains `stopNumber` if a specific point was clicked. Guide's preview sheet is shown.                                                 |
...
| `guideStepShown`          | `guideId: number, stopNumber: number`  | A point in a guide becomes the active point. User can trigger this in two ways: a) a guide's initial activation, or b) user navigates between stops (using previous/next buttons). |
